...
def process_knowledge_query(message: str, intent: str, surface: str) -> str:
    ...
    try:
        ...
        # Build context from knowledge base (with confidence=1.0 as default)
        context = build_context(intent, confidence=1.0, surface=surface)
...
def render_response(raw_response: str, surface: str, intent: str, confidence: float) -> str:
    ...
    """
    try:
        from renderer import render_response as render_fn
        
        # Use renderer's render_response function
        response = render_fn(
            raw_response,
            surface=surface,
            intent=intent,
